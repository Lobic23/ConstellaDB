use sqlparser::ast::{
    Statement as SqlStatement, Expr as SqlExpr, BinaryOperator, SetExpr,
    DataType as SqlDataType, Value as SqlValue, FromTable, CharacterLength,
};
use sqlparser::dialect::GenericDialect;
use sqlparser::parser::Parser as SqlParser;

use db_module::{Table, Attr, Type, Value, Entity, Data, Condition, Operator};
...
fn convert_statement(stmt: SqlStatement) -> Result<Command, CmdError> {
    match stmt {
        SqlStatement::CreateTable { name, columns, .. } => {
            let attrs = columns.into_iter()
                .map(|c| Ok(Attr {
                    name: c.name.value,
                    data_type: convert_data_type(c.data_type)?,
                }))
                .collect::<Result<Vec<_>, CmdError>>()?;
            Ok(Command::CreateTable(Table { name: name.to_string(), attrs }))
        }
        ...
        SqlStatement::Insert { table_name, columns, source, .. } => {
            ...
            let source = source
                .ok_or_else(|| CmdError::Syntax("INSERT requires VALUES".into()))?;
            ...
            let data = col_names.into_iter().zip(row)
                .map(|(name, value)| Data { name, value })
                .collect();
            Ok(Command::Insert(Entity { of: table_name.to_string(), data }))
        }
        SqlStatement::Query(query) => convert_select(*query),
        SqlStatement::Update { table, assignments, selection, .. } => {
            let updates = assignments.into_iter().map(|a| {
...
fn convert_select(query: sqlparser::ast::Query) -> Result<Command, CmdError> {
    ...
    let attrs = select.projection.into_iter().map(|item| match item {
    ...
    Ok(Command::Select { table, attrs, conditions })
}
    ...
    match expr {
        SqlExpr::BinaryOp { left, op, right } => match op {
            ...
            | BinaryOperator::Gt | BinaryOperator::LtEq | BinaryOperator::GtEq => {
                ...
                Ok(Condition::Compare { attr, value, op: cmp_op })
            }
            other => Err(CmdError::UnsupportedExpr(format!("{:?}", other))),
        },
