...
impl Engine {
  ...
  pub fn alter_table(&mut self, table_name: &str, op: AlterOp) -> Result<(), String> {
    let table = self
    ...
    match op {
      AlterOp::AddColumn(attr) => self.add_column(&table, attr),
      AlterOp::DropColumn(name) => self.drop_column(&table, &name),
      AlterOp::RenameColumn { from, to } => self.rename_column(&table, &from, &to),
      AlterOp::ModifyColumnType { name, new_type } => {
        self.modify_column_type(&table, &name, new_type)
      }
    }
  }

  fn add_column(&mut self, table: &Table, attr: Attr) -> Result<(), String> {
  ...
  fn drop_column(&mut self, table: &Table, col_name: &str) -> Result<(), String> {
  ...
  fn rename_column(&mut self, table: &Table, from: &str, to: &str) -> Result<(), String> {
  ...
  fn modify_column_type(
    ...
    let values = self.load_column(table, attr)?;
