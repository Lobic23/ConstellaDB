...
mod tests {
    ...
    fn alter_drop_column_succeeds() {
        ...
        let result =
            engine.alter_table("user_table", AlterOp::DropColumn("password".to_string()));

        assert!(result.is_ok(), "{:?}", result);

        assert!(engine.select("user_table", vec!["password"], vec![]).is_err());
    }
    ...
    fn alter_drop_nonexistent_column_errors() {
        ...
        let result =
            engine.alter_table("user_table", AlterOp::DropColumn("no_col".to_string()));
    ...
    fn alter_drop_last_column_errors() {
        ...
        let result =
            engine.alter_table("single_col", AlterOp::DropColumn("only".to_string()));
