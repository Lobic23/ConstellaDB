...
use constella_db::modules::{
  db::Entity,
  protocol::{BincodeSerializer, Message, MessageType, ProtocolHandler, ResponseData},
};
...
async fn execute_sql(handler: &mut ProtocolHandler, sql: &str, node_id: &str) -> bool {
  let msg = Message::new(
        ...
        if let Some(data) = data {
          match data {
            ...
            ResponseData::Count(count) => {
              println!("Processed on {} rows", count)
            }
          }
        }

        println!("{} {:.2?}\n", "Time:".blue(), start.elapsed());
      }
...
async fn execute_file(handler: &mut ProtocolHandler, path: &str, node: &str) {
    ...
    match rl.readline("constella_db> ") {
      Ok(line) => {
        ...
        if let Some(file) = cmd.strip_prefix("run-file ") {
          execute_file(&mut handler, file.trim(), &node_id).await;
          continue;
        }

        execute_sql(&mut handler, cmd, &node_id).await;
      }

      Err(ReadlineError::Interrupted) => {
        println!();
        println!("{}", "Input cancelled. Press Ctrl+D to quit.".yellow());
        continue;
      }
